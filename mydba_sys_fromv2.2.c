...
int check_list( const u_char * buf )
{
	...
	buf_check_list = buf;
	//pthread_mutex_lock(&mutex_config);
	int count_bad = 0, count_s = 0;
	char check_policy_src[2000] = "";
	DL_FOREACH(config_head,config_elt)
	{
		if (config_elt->status == 1)	//该条生效启用
		{	//v1 is in config line, v2 is in v2 config after userkey
			if (config_elt->db_port[0] == buf_check_list[36] && config_elt->db_port[1] ==buf_check_list[37])
			{	//v1, direction 2, check port first
				...
				{	//v1, direction 2, check ip address
					count_bad++;
					switch( config_elt->db_type )
					{
						printf("config_elt->db_type is %d\n", config_elt->db_type);
		...
		//printf("db to visitor, db_type is %d\n", db_type_tmp);
			...
			{	//v1, check direction 1, check port first
				...
				{	//v1, check direction 1, check ip address
					count_bad++;
					switch( config_elt->db_type )
					{
						printf("config_elt->db_type is %d\n", config_elt->db_type);
					...
					direction_tmp = 1;
					//pthread_mutex_unlock(&mutex_config);
		//printf("visit db, db_type is %d\n", db_type_tmp);
					return (db_type_tmp * direction_tmp);
				}
			}

			else if ( config_elt->userkey[0] == buf_check_list[42] && config_elt->userkey[1] == buf_check_list[43] )
...
//need to add judge here.
//such as jump syslog head, add timestamp, type and so on.
...
//add risk_id
				...
				{	continue;
					memcpy(sys_content+13, "00", 2);
				}

		//		int check_if_log_result = check_if_log( buf_check_list + before_data_head + position_to_check, syslog_ip_port, config_elt->cus, config_elt->key_ipp, config_elt->if_custom);
		//		printf("check_if_log_result is %d\n", check_if_log_result);
		//		if( check_if_log_result == 0 )	//1 means have, 0 means no
		///		{
		//			continue;
		//		}
//printf("~~~~~~~~~~~~~~~~~~~~~~~");
				//printf("timestamp is %s\n", timestamp);
				//printf("sizeof(sys_content) is %d\n\n", sizeof(sys_content));
				//printf("sys_length is %d\n\n", sys_length);
		...
		//printf("a5\n");
				...
				//memcpy(sys_content+18, buf_check_list+82, sys_length-48);	// sys_content is a global varible, set a constant length
				//printf("sys_content is %s\n", sys_content);

		//memcpy(sys_content+18, buf_check_list + before_data_head + 40, 40);
				...
				memcpy(sys_content+78, buf_check_list + before_data_head + 70, 10);



				//memcpy(sys_content+58, buf_check_list+122, sys_length-80);

		//printf("a7\n");
				//printf("type is %02x,%02x\n", *(buf_check_list + before_data_head+70), *(buf_check_list + before_data_head+71) );
				//if sm version, 70 and 71 will be 01 or 02, normal version, that will be set 00, that is 0x30 in hex.
				if( *(buf_check_list + before_data_head+70) == 0x30 && *(buf_check_list + before_data_head+71) == 0x30 )
				{	//printf("aaaa\n");
				...
				n = sprintf(sys_array, "%s", sys_content);
				print_buf(sys_array, n, &syslog_buffer);
				n = sprintf(sys_array, "\n");
				print_buf(sys_array, n, &syslog_buffer);
				//n = sprintf(sys_array, "%s", sys_content);
...
{	int  ovector[OVECCOUNT];
	...
	int log_len = src_len;
//printf("p0^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n"); 
//printf("src_len is %d, src is %s\n", src_len, src);
//printf("22the_ip_port is %s\n", the_ip_port);
	//pthread_mutex_lock(&mutex_policy); 
...
int main(int argc, char *argv[])
{
	...
	pcap_t * device = pcap_open_live(devStr, 65535, 0, 0, errBuf);
