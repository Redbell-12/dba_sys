...
20200811 v2.4 add function check_if_log_sys(), add custom check and ipport check.
	...
	while ( fgets (mystring , 2000 , pFile) != NULL ){
		if(!memcmp(mystring,"[dba_rule_zcp]",13)){
			if ( (policy_name = (dba_policy_struct*)malloc(sizeof(dba_policy_struct))) != NULL){
				...
				{	char policy_id[37];			//uuid
					memcpy(policy_id,mystring+24,36);	//uuid
					//policy_name->id=atoi(policy_id);
					memcpy(policy_name->uuid,mystring+24,36);	//uuid
					policy_name->uuid[36] = '\0';			//uuid

					printf("policy_name->id:%s ",policy_name->uuid);
					...
					policy_name->log_type=atoi(mystring+29+33);	//uuid
					printf("policy_name->log_type:%d ",policy_name->log_type);
					policy_name->action=atoi(mystring+32+33);	//uuid
					policy_name->str_len=atoi(mystring+35+33);	//uuid
...
//only if ip is match and status is off, then we don't record.
//if no ip, the ipport will be added. when added, and the status is off, we won't record.
int check_if_log_sys(char *syslog_ip_port, char *check_policy_src, int len_of_custom)
{	int return_code = 0;	//0 means no record, 1 means can record.
	int custom_rc, i;
	int  ovector[OVECCOUNT];
	printf("len_of_custom is %d\n",len_of_custom);
	printf("check_policy_src is %s\n", check_policy_src);

	DL_FOREACH(config_head,config_elt)
	{	if(!memcmp(config_elt->key_ipp, syslog_ip_port, 12))
		{	if ( !memcmp(config_elt->status, status_off, 2) )
			{	return_code = 0;
				return 0;	//status is off, no need to do other judge.
			}
			else
			{	return_code = 1;
			}
			...
			{	return_code = 1;
				return 1;	//no need to check, return 1.
			}
			else	//if_custom is 1, need to check whether if match.
			{	if ( ( custom_rc = pcre_exec(config_elt->cus, NULL, check_policy_src, len_of_custom, 0, 0, ovector, OVECCOUNT)) != PCRE_ERROR_NOMATCH )
...
//1578667281|||02|||000000000000|||000000000000|||c0a8930f05f1|||000000000000|||00|||00|||0|||true|||2020-01-17 07:37:28.2250757|||53|||0|||0|||WIN-9K6K2JE6MSA\Administrator|||123|||dbo.test|||select * from dbo.test
				...
				//memset(check_policy_src0,'\0',sizeof(check_policy_src0));
				memcpy(check_policy_src0, buf_check_list + before_data_head + position_to_check, sys_length - position_to_check - layer3_udp_len);
				if_record = check_if_log_sys(syslog_ip_port, check_policy_src0, sys_length - position_to_check - layer3_udp_len);
				if ( if_record == 0 )
				{	return 0;
				}
...
//0	    10   15 18		30 33	       45 48	      60 63	     75 78   83   88 91 94
				...
				{	//printf("aaaa\n");
					...
					memcpy(check_policy_src, buf_check_list + before_data_head + position_to_check, sys_length - position_to_check - layer3_udp_len);
				//uuid	sys_check_result = check_policy_dba( check_policy_src, syslog_ip_port, sys_length - position_to_check - layer3_udp_len );
					check_policy_dba( check_policy_src, syslog_ip_port, sys_length - position_to_check - layer3_udp_len );
					//printf("sys_check_result is %03d\n", sys_check_result);
					//sprintf(sys_check_result_p, "%s", sys_check_result);

				//	memcpy(sys_content+88, sys_check_result_p, 3);	//id 3
					memcpy(sys_content+88, strategy_info.uuid, 36);	//uuid
				}
				else	//01 or 02 or 03
				{	//in else means sm
					memcpy(sys_content+88, CORRECT, 36);		//uuid
				}
//1578667281|||02|||000000000000|||000000000000|||c0a8930f05f1|||000000000000|||00|||00|||000|||true|||2020-01-17 07:37:28.2250757|||53|||0|||0|||WIN-9K6K2JE6MSA\Administrator|||123|||dbo.test|||select * from dbo.test
//0	    10   15 18		30 33	       45 48	      60 63	     75 78   83   88 91 94
				
				memcpy(sys_content+91+33, buf_check_list + before_data_head + 83, sys_length - 83 - layer3_udp_len);
...
{	int  ovector[OVECCOUNT];
	int  rc, i;
	memcpy(strategy_info.uuid, CORRECT, 36);
	...
	{	if(!memcmp(policy_elt->key_ip, the_ip_port, 12) || !memcmp(policy_elt->key_ip,key_ip_zero,12))
		{	//printf("p3\n");
			...
			{	printf("policy access system_find_OK!!!!,id:%s,action:%d\n",policy_elt->uuid,policy_elt->action);
...
int main(int argc, char *argv[])
{
	...
	DL_FOREACH(config_head,config_elt){		
		...
		printf("config_name->userkey is %s, config_name->staus is %s\n", config_elt->userkey, config_elt->status);
	}
	//pthread_mutex_unlock(&mutex_config);
